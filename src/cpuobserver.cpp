...
CPUObserver::CPUObserver(boost::shared_ptr<SMConfig> cfg,
    ...
    if (this->procStreamPath == "") this->procStreamPath = "/proc/loadavg";
    try {
        this->msToWait = std::stoi(
            this->cfg->getConfigValue("/config/observer/sysstat/cpu/pollTime"));
    } catch (const std::exception &ex) {
        this->log->error("Can not parse \"observer/sysstat/cpu/pollTime\" " +
                         std::string(ex.what()));
        this->msToWait = 1000;
    }
    try {
    ...
    this->log->debug("CPUObserver Object instantiated");
}
...
void CPUObserver::checkStreamData() {
    this->log->debug("Checking stream data...");
    float avg5, avg15;
    if (this->cpuLoad.size() == 5) {
        try {
            ...
            this->log->error("Can not cast: " + this->cpuLoad[1] + ", " +
                             this->cpuLoad[2] +
...
void CPUObserver::composeMailMessage(std::string &msg) {
    BOOST_FOREACH (const std::string &s, this->cpuLoad) { msg += s + " "; }
    this->log->debug(msg);
